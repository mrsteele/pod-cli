 ...
 */
export async function configWizard(options: { postinstall?: boolean } = {}): Promise<void> {
  ...
  try {
    ...
    if (isPostinstall) {
      ...
      if (!proceed) {
        console.log('');
        console.log(chalk.dim('Skipping setup. You can run "pod config" later to configure.'));
        rl.close();
        return;
      }
      console.log('');
...
export async function postinstallSetup(): Promise<void> {
  ...
  await configWizard({ postinstall: true });
}
